...
(function (window, undefined) {
  var Idm = function (config) {
    ...
    for (var i = 0; i < scripts.length; i++) {
      if ((scripts[i].src.indexOf('idm.js') !== -1 || scripts[i].src.indexOf('idm.min.js') !== -1) && scripts[i].src.indexOf('version=') !== -1) {
    ...
    kc.isTokenExpired = function (minValidity) {
      ...
      if (kc.timeSkew == null) {
        console.info('[idm] Unable to determine if token is expired as timeskew is not set')
        return true
      }
    ...
    kc.updateToken = function (minValidity) {
      ...
      var exec = function () {
        ...
        if (minValidity == -1) {
          refreshToken = true
          console.info('[idm] Refreshing token: forced refresh')
        } else if (!kc.tokenParsed || kc.isTokenExpired(minValidity)) {
          refreshToken = true
          console.info('[idm] Refreshing token: token expired')
        }
        ...
        if (!refreshToken) {
          ...
          if (refreshQueue.length == 1) {
            ...
            req.onreadystatechange = function () {
              if (req.readyState == 4) {
                if (req.status == 200) {
                  console.info('[idm] Token refreshed')
                  ...
                  console.warn('[idm] Failed to refresh token')
    ...
    function processCallback (oauth, promise) {
      ...
      function authSuccess (accessToken, refreshToken, idToken, fulfillPromise) {
        ...
        if (useNonce && ((kc.tokenParsed && kc.tokenParsed.nonce != oauth.storedNonce) ||
          ...
          console.info('[idm] Invalid nonce, clearing token')
          kc.clearToken()
          promise && promise.setError()
        } else {
          if (fulfillPromise) {
      ...
      if (configUrl) {
        ...
        if (!oidcProvider) {
          if (!config['url']) {
            var scripts = document.getElementsByTagName('script')
            for (var i = 0; i < scripts.length; i++) {
              if (scripts[i].src.match(/.*idm\.js/)) {
                config.url = scripts[i].src.substr(0, scripts[i].src.indexOf('/js/idm.js'))
                break
              }
            }
          }
    ...
    function setToken (token, refreshToken, idToken, timeLocal) {
      ...
      if (token) {
        ...
        if (kc.timeSkew != null) {
          console.info('[idm] Estimated time difference between browser and server is ' + kc.timeSkew + ' seconds')

          if (kc.onTokenExpired) {
            var expiresIn = (kc.tokenParsed['exp'] - (new Date().getTime() / 1000) + kc.timeSkew) * 1000
            console.info('[idm] Token expires in ' + Math.round(expiresIn / 1000) + ' s')
            if (expiresIn <= 0) {
              kc.onTokenExpired()
            } else {
  ...
  if (typeof module === 'object' && module && typeof module.exports === 'object') {
    ...
    if (typeof define === 'function' && define.amd) {
      define('keycloak', [], function () { return Idm })
    }
  }
})(window)
